...
//        let dataEncryptMethod: DataEncryptMethod?
    ...
    static func start(wait: Bool) {
        if ms_server_current_status() == ms_server_status_idle {
            let c_url = NSTemporaryDirectory().cString(using: .utf8)
            ms_asnyc_start(8090, c_url)
        }
        if wait {
            while ms_server_current_status() != ms_server_status_running {
                sleep(1)
            }
        }
    }
